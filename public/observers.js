...
reboot</code></pre>
  ...
  function openObserverSlideOver(hashHref) {
    ...
    var naiveChipHTML = window.ObserversNaiveChip ? window.ObserversNaiveChip.render(o) : '';
    content.innerHTML =
      (naiveChipHTML ? '<div style="margin-bottom:10px">' + naiveChipHTML
          + ' <span class="text-muted">Clock is naive — per-packet timing clamped to ingest time.</span></div>' : '') +
